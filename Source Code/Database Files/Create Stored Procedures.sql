...
DELIMITER //
CREATE procedure spUpdateTimesheetEntriesForPerson (IN ID int, IN ClockIn DateTime, IN ClockOut Datetime, IN UnpaidTimeSpanTicks BIGINT, IN PaidTimeSpanTicks BIGINT)
BEGIN
...
--    set T_id=ID,clock_in=ClockIn,clock_out=ClockOut,unpaidTicks=UnpaidTimeSpanTicks,paidTicks=PaidTimeSpanTicks;
	...
	INSERT INTO TimesheetEntries(ID,ClockIn,ClockOut,UnpaidTime,PaidTime,PersonID)
    VALUES (ID,ClockIn,ClockOut,UnpaidTimeSpanTicks,PaidTimeSpanTicks,p_ID)
    ON DUPLICATE KEY UPDATE ClockIn = ClockIn, ClockOut = ClockOut, UnpaidTime = UnpaidTimeSpanTicks, PaidTime = PaidTimeSpanTicks;
